...
void drawPoints(cv::Mat& image, vector<cv::Point> points,cv::Scalar color)
{
  for( vector<cv::Point>::const_iterator i = points.begin(), ie = points.end(); i != ie; ++i )
      {
...
int main()
{
     ...
     for(int index = 0; index < image_num; index++)
    {
         ...
         if( index == 0)
           {
             vector<cv::Point> pts;
             for(int u=20; u< gray_image.cols-20; u++)
               {
                 for(int v=20; v<gray_image.rows-20; v++)
                   {
                     Vector2d delta(gray_image.at<uchar>(v+1,u)-gray_image.at<uchar>(v-1,u),
                                               gray_image.at<uchar>(v,u+1) - gray_image.at<uchar>(v,u+1));
                     if (delta.norm() < 50)
                       continue;

                     ushort d = depth_image.ptr<ushort>(v)[u];
                     if(d == 0)
                       continue;

                     Eigen::Vector3d p3d;
                     p3d = imageTo3d(u, v, d, camera);
                     float imagegray = float( gray_image.at<uchar>(v, u) );
                     pts.push_back(cv::Point(u,v));
             ...
             cv::Mat imgShow ;
             color_image.copyTo(imgShow);
             drawPoints(imgShow, pts,cv::Scalar::all(-1));
